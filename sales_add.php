...
$sales_id = mysqli_real_escape_string($link,$_POST['sales_id']);
...
list($customer_name) = mysqli_fetch_row(mysqli_query($link,"select `customer_name` from customer_tbl where customer_id='$customer_id'"));
...
mysqli_query($link,"INSERT INTO `salesdetails_tbl` (`id`, `salesdetails_id`, `company_id`, `branch_id`, `sales_id`, `sales_date`, `po_no`, `po_date`, `invoice_no`, `invoice_date`, `customer_id`, `customer_name`, `product_id`, `productcode`, `barcode`, `product_name`, `qty`, `rate`, `unit`, `line_discount_type`, `line_discount`, `line_discount_total`, `tax`, `tax_total`, `line_total`, `batch_no`, `expiry_date`, `sort_order`, `status`, `ip_address`, `browser`, `date`, `created_by`, `approved_by`) VALUES (NULL, '$salesdetails_id', '$company_id', '$branch_id', '$sales_id', '$invoice_date', '$po_no', '$po_date', '$invoice_no', '$invoice_date', '$customer_id', '$customer_name', '$product_id', '$productcode', '$barcode', '$product_name', '$qty', '$rate', '$unit', '$line_discount_type', '$line_discount', '$line_discount_total', '$tax', '$tax_total', '$line_total', '$batch_no', '$expiry_date', '$sort_order', 'active', '$ip_address', '$browser', '$date', '$created_by', '$approved_by')");
...
//Stock Module
$sel_qty = mysqli_query($link,"select `product_id`,`qty` from salesdetails_tbl where `sales_id`='$sales_id'");
while($row_qty = mysqli_fetch_object($sel_qty)){
	$oldproduct[] = $row_qty->product_id.",".$row_qty->qty;
}
//End Stock Module

mysqli_query($link,"delete from salesdetails_tbl where `sales_id`='$sales_id'");
$product_id1 = $_POST['product_id'];
$i=0;
foreach($product_id1 as $product_id2){
$max_id = maxOfAll("id","salesdetails_tbl");
              ...
              <div class="container box-bg">
                <div class="box">
                    <div class="box-header">
                      <div class="row">
                        <div class="col-lg-6"><h3 class="box-heading"> Sales Details <small>Add / Update  Details</small></h3></h3></div>
                        <div class="col-lg-6">
                          <div class="breadcrumb">
                            <a href="index.php" class="breadcrumb_a">Home</a> 
                            <i class="fa fa-angle-double-right angle_double_right"></i>
                            <a href="sales_view.php" class="breadcrumb_a">Sales Details </a> 
                            <i class="fa fa-angle-double-right angle_double_right" aria-hidden="true"></i>
                            <a href="#" class="breadcrumb_a">Add / Update Sales Details </a>
                          </div>
                        </div>
                      </div>
                    </div>
...
<?php
	$sales_id = mysqli_real_escape_string($link,$_GET['sales_id']);
	$update = mysqli_real_escape_string($link,$_GET['update']);
	if($update == "yes"){
	$sel_row = mysqli_query($link,"select * from sales_tbl where `sales_id`='$sales_id'");
	$row = mysqli_fetch_object($sel_row);
	$upload_id = $row->sales_id;
  ...
  <div class="alert alert-success alert-dismissible">
   ...
   <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-pad">
			...
			<tbody class="">
			<tr class="sales_details"></tr>
			  <?php
			  if($update == "yes"){
			  $i = 0;
			  $sel = mysqli_query($link,"select * from salesdetails_tbl where sales_id='$sales_id'");
			  while($row_sales = mysqli_fetch_object($sel)){
			  ?>
			  <tr class="sales_details" id="<?php echo $row_sales->product_id; ?>">
				<td width="30%">
          ...
          success:function(data){
			...
			if(batch_stock == "yes"){
				...
				$("#stock-list").one("click","tr.rows", function(e){
					...
					var rowcount = $('.sales_details').length;
					//alert(rowcount);
...
<script type="text/javascript" src="assets/js/salesdetails_tbl.js"></script>
