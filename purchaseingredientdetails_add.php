...
if($_POST['submit'] == "Submit"){
    ...
    $purchaseingredient_id = mysqli_real_escape_string($link,$_POST['purchaseingredient_id']);
    $max_id = maxOfAll("id","purchaseingredient_tbl");
    $max_id=$max_id+1;
    $purchaseingredient_id="pur-".$max_id;	
    $purchaseingredient_date = mysqli_real_escape_string($link,$_POST['purchaseingredient_date']);
    ...
    $type = "png";
    $rand = rand();
    $file = "barcode/purchaseingredient/$purchaseingredient_id"."_".date("d-m-Y")."_".$rand;
    ...
    $max_id = maxOfAll("id","purchaseingredientdetails_tbl");
    $max_id=$max_id+1;
    $purchaseingredientdetails_id="pd-".$max_id;	
    ...
    mysqli_query($link,"INSERT INTO `purchaseingredientdetails_tbl` (`id`, `purchaseingredientdetails_id`, `company_id`, `branch_id`, `purchaseingredient_id`, `purchaseingredient_date`, `po_no`, `po_date`, `invoice_no`, `invoice_date`, `vendor_id`, `vendor_name`, `ingredient_id`, `ingredientcode`, `barcode`, `ingredient_name`, `qty`, `rate`, `unit`, `line_discount_type`, `line_discount`, `line_discount_total`, `mrp_price`, `selling_price`, `wholesale_price`, `tax`, `tax_total`, `line_total`, `batch_no`, `expiry_date`, `sort_order`, `status`, `ip_address`, `browser`, `date`, `created_by`, `approved_by`) VALUES (NULL, '$purchaseingredientdetails_id', '$company_id', '$branch_id', '$purchaseingredient_id', '$purchaseingredient_date', '$po_no', '$po_date', '$invoice_no', '$invoice_date', '$vendor_id', '$vendor_name', '$ingredient_id', '$ingredientcode', '$barcode', '$ingredient_name', '$qty', '$rate', '$unit', '$line_discount_type', '$line_discount', '$line_discount_total', '$mrp_price', '$selling_price', '$wholesale_price', '$tax', '$tax_total', '$line_total', '$batch_no', '$expiry_date', '$sort_order', 'active', '$ip_address', '$browser', '$date', '$created_by', '$approved_by')");
    ...
    $type = "debit";
    $payment_type = "purchaseingredient";//sales
    mysqli_query($link,"UPDATE `accounts_tbl` SET `company_id` = '$company_id', `branch_id` = '$branch_id', `date` = '$date', `type` = '$type', `amount` = '$paid_amount', `payer` = '$vendor_name', `category` = '$payment_type', `payment_mode` = '$paymodes', `card_no` = '$card_no', `cheque_no` = '$cheque_no', `cheque_dt` = '$cheque_dt', `trans_no` = '$trans_no', `description` = '$payment_desc', `invoice_no` = '$invoice_no', `subinvoice_no` = '$subinvoice_no', `bank_account` = '$bank_account', `sort_order` = '$sort_order', `status` = '$status', `ip_address` = '$ip_address', `browser` = '$browser', `created_by` = '$created_by', `approved_by` = '$approved_by' WHERE `autoid`='$payment_id'");
    foreach($payment_mode as $paymentmode){
    if($paymentmode == "cash"){
    $tender = mysqli_real_escape_string($link,$_POST['cash_tender']);	
    }
    ...
    //Stock Module
    $sel_qty = mysqli_query($link,"select `ingredient_id`,`qty` from purchaseingredientdetails_tbl where `purchaseingredient_id`='$purchaseingredient_id'");
    while($row_qty = mysqli_fetch_object($sel_qty)){
        $oldingredient[] = $row_qty->ingredient_id.",".$row_qty->qty;
    }
    //End Stock Module
    
    mysqli_query($link,"delete from purchaseingredientdetails_tbl where `purchaseingredient_id`='$purchaseingredient_id'");
    $ingredient_id1 = $_POST['ingredient_id'];
    $i=0;
    foreach($ingredient_id1 as $ingredient_id2){
    $max_id = maxOfAll("id","purchaseingredientdetails_tbl");
                ...
                <div class="box">
                      ...
                      <div class="row">
                        ...
                        <div class="col-lg-6">
                          <div class="breadcrumb">
                            ...
                            <a href="purchaseingredient_view.php" class="breadcrumb_a">Purchase Ingredient</a> 
                    ...
                    <div class="box-body">
                      ...
                      <form method="post" class="padding-10" enctype="multipart/form-data">
                        <?php
$purchaseingredient_id = mysqli_real_escape_string($link,$_GET['purchaseingredient_id']);
$update = mysqli_real_escape_string($link,$_GET['update']);
if($update == "yes"){
$sel_row = mysqli_query($link,"select * from purchaseingredient_tbl where `purchaseingredient_id`='$purchaseingredient_id'");
$row = mysqli_fetch_object($sel_row);
$upload_id = $row->purchaseingredient_id;
...
<input type="hidden" class="form-control" name="id" id="id" placeholder="Enter the Id" value="<?php echo $row->id; ?>" readonly="readonly">
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        ...
        <label class="margin-left-10" for="industry_name">Purchase Ingredient Id</label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
        <input type="text" class="form-control" name="purchaseingredient_id" id="purchaseingredient_id" placeholder="Enter the Purchase Id" value="<?php echo $upload_id; ?>" readonly="readonly">
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 no-pad">
        <label class="margin-left-10" for="industry_name">Purchase Date</label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
        <input type="date" class="form-control" name="purchaseingredient_date" id="purchaseingredient_date" placeholder="Enter the Purchase Date" value="<?php echo $row->purchaseingredient_date; ?>" required="required">
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-pad">
		<table class="table table-bordered mg-none table1">
			...
			<tbody class="">
			<tr class="purchaseingredient_details"></tr>
			  <?php
			  if($update == "yes"){
			  $i = 0;
			  $sel = mysqli_query($link,"select * from purchaseingredientdetails_tbl where purchaseingredient_id='$purchaseingredient_id'");
			  while($row_purchaseingredient = mysqli_fetch_object($sel)){
			  ?>
			  <tr class="purchaseingredient_details" id="<?php echo $row_purchaseingredient->ingredient_id; ?>">
				<td width="25%">
					<input class="form-control" type="hidden" name="ingredient_id[]" id="ingredient_id<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->ingredient_id; ?>">
					<input class="form-control" type="hidden" name="ingredientcode[]" id="ingredientcode<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->ingredientcode; ?>">
					<input class="form-control" type="hidden" name="barcode[]" id="barcode<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->barcode; ?>">
					<textarea class="form-control" name="ingredient_name[]" id="ingredient_name<?php echo $i; ?>"><?php echo $row_purchaseingredient->ingredient_name; ?></textarea>
					<table>
						<tr>
						...
						<tr>
							<td><input class="form-control" type="date" name="expiry_date[]" id="expiry_date<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->expiry_date; ?>"></td>
							<td><input class="form-control" type="text" name="batch_no[]" id="batch_no<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->batch_no; ?>"></td>
						</tr>
					</table>
				</td>
				<td width="5%">
				<input class="form-control qty" type="text" name="qty[]" id="qty<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->qty; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="rate[]" id="rate<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->rate; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="unit[]" id="unit<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->unit; ?>">
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="mrp_price[]" id="mrp_price<?php echo $i; ?>" placeholder="MRP Price" value="<?php echo $row_purchaseingredient->mrp_price; ?>">
				<input class="form-control" type="text" name="selling_price[]" id="selling_price<?php echo $i; ?>" placeholder="Selling Price" value="<?php echo $row_purchaseingredient->selling_price; ?>">
				<input class="form-control" type="text" name="wholesale_price[]" id="wholesale_price<?php echo $i; ?>" placeholder="Wholesale" value="<?php echo $row_purchaseingredient->wholesale_price; ?>">
				</td>
				<td width="10%">
				<select name="line_discount_type[]" id="line_discount_type<?php echo $i; ?>" class="form-control" onchange="update_linetotal(<?php echo $i; ?>)">
						<option value="no discount" <?php if($row_purchaseingredient->line_discount_type == "no discount"){ echo "selected"; } ?>>No Discount</option>
						<option value="flatrate" <?php if($row_purchaseingredient->line_discount_type == "flatrate"){ echo "selected"; } ?>>Flat Rate</option>
						<option value="percentage" <?php if($row_purchaseingredient->line_discount_type == "percentage"){ echo "selected"; } ?>>Percentage</option>
				</select>
				<input class="form-control" type="text" name="line_discount[]" id="line_discount<?php echo $i; ?>" placeholder="Value" value="<?php echo $row_purchaseingredient->line_discount; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				<input class="form-control linediscount" type="text" name="line_discount_total[]" id="line_discount_total<?php echo $i; ?>" placeholder="Total" value="<?php echo $row_purchaseingredient->line_discount_total; ?>" readonly>
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="tax[]" id="tax<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->tax; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				<input class="form-control taxvalue" type="text" name="tax_total[]" id="tax_total<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->tax_total; ?>">
				</td>
				<td width="10%">
				<input class="form-control price" type="text" name="line_total[]" id="line_total<?php echo $i; ?>" value="<?php echo $row_purchaseingredient->line_total; ?>">
				</td>
				<td>
				<a class="purchaseingredientdel" href="javascript:;" title="Remove row"><i class="fa fa-close"></i></a>
				</td>
			</tr>
				<?php $i++; } }?>
			</tbody>
...
$('#po_date').val(today);
$('#purchaseingredient_date').val(today);
...
function get_ingredient(){
	...
	$.ajax({
          ...
          success:function(data){
			...
			if(count < 1){
				var i = $('.purchaseingredient_details').length;
				addrow(i,ingredient_id,ingredientcode,ingredient_name,barcode,unit,tax,price_changeable,stockable);
			}
			else
			{
				var rowcount = $('.purchaseingredient_details').length;
...
<script type="text/javascript" src="assets/js/purchaseingredientdetails_tbl.js"></script>
