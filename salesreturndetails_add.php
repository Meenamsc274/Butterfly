...
$salesreturn_id = mysqli_real_escape_string($link,$_POST['salesreturn_id']);
$max_id = maxOfAll("id","salesreturn_tbl");
$max_id=$max_id+1;
$salesreturn_id="sal-".$max_id;	
...
$type = "png";
$rand = rand();
$file = "barcode/salesreturn/$salesreturn_id"."_".date("d-m-Y")."_".$rand;
...
$max_id = maxOfAll("id","salesreturndetails_tbl");
$max_id=$max_id+1;
$salesreturndetails_id="pd-".$max_id;	
...
mysqli_query($link,"INSERT INTO `salesreturndetails_tbl` (`id`, `salesreturndetails_id`, `company_id`, `branch_id`, `salesreturn_id`, `salesreturn_date`, `po_no`, `po_date`, `invoice_no`, `invoice_date`, `customer_id`, `customer_name`, `product_id`, `productcode`, `barcode`, `product_name`, `qty`, `rate`, `unit`, `line_discount_type`, `line_discount`, `line_discount_total`, `tax`, `tax_total`, `line_total`, `batch_no`, `expiry_date`, `sort_order`, `status`, `ip_address`, `browser`, `date`, `created_by`, `approved_by`) VALUES (NULL, '$salesreturndetails_id', '$company_id', '$branch_id', '$salesreturn_id', '$invoice_date', '$po_no', '$po_date', '$invoice_no', '$invoice_date', '$customer_id', '$customer_name', '$product_id', '$productcode', '$barcode', '$product_name', '$qty', '$rate', '$unit', '$line_discount_type', '$line_discount', '$line_discount_total', '$tax', '$tax_total', '$line_total', '$batch_no', '$expiry_date', '$sort_order', 'active', '$ip_address', '$browser', '$date', '$created_by', '$approved_by')");
...
$sort_order = mysqli_real_escape_string($link,$_POST['sort_order']);
$status = mysqli_real_escape_string($link,$_POST['status']);
if(mysqli_query($link,"UPDATE `salesreturn_tbl` SET `company_id` = '$company_id', `branch_id` = '$branch_id', `salesreturn_id` = '$salesreturn_id', `salesreturn_date` = '$invoice_date', `po_no` = '$po_no', `po_date` = '$po_date', `invoice_no` = '$invoice_no', `invoice_date` = '$invoice_date', `customer_id` = '$customer_id', `customer_name` = '$customer_name', `financial_year` = '$financial_year', `total_lineitems` = '$total_lineitems', `total_qty` = '$total_qty', `total_tax` = '$total_tax', `total_linediscount` = '$total_linediscount', `sub_total` = '$sub_total', `flatrate_discount` = '$flatrate_discount', `percentage_discount` = '$percentage_discount', `total_overalldiscount` = '$total_overalldiscount', `grand_total` = '$grand_total', `paid_amount` = '$paid_amount', `payment_id` = '$payment_id', `balance_amount` = '$balance_amount', `description` = '$payment_desc', `ip_address` = '$ip_address', `browser` = '$browser', `date` = '$date', `created_by` = '$created_by', `approved_by` = '$approved_by' WHERE `salesreturn_id`='$salesreturn_id'")){
...
$type = "credit";
$payment_type = "sales";//sales
mysqli_query($link,"UPDATE `accounts_tbl` SET `company_id` = '$company_id', `branch_id` = '$branch_id', `date` = '$date', `type` = '$type', `amount` = '$paid_amount', `payer` = '$customer_name', `category` = '$payment_type', `payment_mode` = '$payment_mode', `card_no` = '$card_no', `cheque_no` = '$cheque_no', `cheque_dt` = '$cheque_dt', `trans_no` = '$trans_no', `description` = '$payment_desc', `invoice_no` = '$invoice_no', `subinvoice_no` = '$subinvoice_no', `bank_account` = '$bank_account', `sort_order` = '$sort_order', `status` = '$status', `ip_address` = '$ip_address', `browser` = '$browser', `created_by` = '$created_by', `approved_by` = '$approved_by' WHERE `autoid`='$payment_id'");
...
//Stock Module
$sel_qty = mysqli_query($link,"select `product_id`,`qty` from salesreturndetails_tbl where `salesreturn_id`='$salesreturn_id'");
while($row_qty = mysqli_fetch_object($sel_qty)){
	$oldproduct[] = $row_qty->product_id.",".$row_qty->qty;
}
//End Stock Module

mysqli_query($link,"delete from salesreturndetails_tbl where `salesreturn_id`='$salesreturn_id'");
$product_id1 = $_POST['product_id'];
$i=0;
foreach($product_id1 as $product_id2){
$max_id = maxOfAll("id","salesreturndetails_tbl");
              ...
              <div class="container box-bg">
                <div class="box">
                    <div class="box-header">
                      <div class="row">
                        <div class="col-lg-6"><h3 class="box-heading"> Sales Return Details <small>Add / Update  Details</small></h3></h3></div>
                        <div class="col-lg-6">
                          <div class="breadcrumb">
                            <a href="index.php" class="breadcrumb_a">Home</a> 
                            <i class="fa fa-angle-double-right angle_double_right"></i>
                            <a href="sales_view.php" class="breadcrumb_a">Sales Return Details </a> 
                            <i class="fa fa-angle-double-right angle_double_right" aria-hidden="true"></i>
                            <a href="#" class="breadcrumb_a">Add / Update Sales Return Details </a>
                          </div>
                        </div>
                      </div>
                    </div>
...
<?php
	$salesreturn_id = mysqli_real_escape_string($link,$_GET['salesreturn_id']);
	$update = mysqli_real_escape_string($link,$_GET['update']);
	if($update == "yes"){
	$sel_row = mysqli_query($link,"select * from salesreturn_tbl where `salesreturn_id`='$salesreturn_id'");
	$row = mysqli_fetch_object($sel_row);
	$upload_id = $row->salesreturn_id;
	...
	$invoice_no = "INV/".$financial_year."/".$max_id;
	}
	?>
					<div class="row mt-3">
                          <div class="col-md-12">
  ...
  <div class="alert alert-success alert-dismissible">
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        ...
        <label class="margin-left-10" for="industry_name">Sales Id</label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
          <input type="text" class="form-control" name="sales_id" id="sales_id" placeholder="Enter the Sales Id" value="<?php echo $upload_id; ?>" readonly="readonly">
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 no-pad">
        <label class="margin-left-10" for="industry_name">Customer </label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
         <select name="customer_id" id="customer_id" class="form-control">
				<?php
				$sel_rw = mysqli_query($link,"select customer_id,customer_name from customer_tbl");
				while($row1 = mysqli_fetch_object($sel_rw)){
				?>
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        ...
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
         <input type="text" class="form-control" name="po_no" id="po_no" placeholder="Enter the Po No" value="<?php echo $row->po_no; ?>">
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        ...
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
         <input type="date" class="form-control" name="po_date" id="po_date" placeholder="Enter the Po Date" value="<?php echo $row->po_date; ?>">
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        ...
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
         <input type="text" class="form-control" name="invoice_no" id="invoice_no" placeholder="Enter the Invoice No" value="<?php echo $invoice_no; ?>" required="required" readonly>
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
   ...
   <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-pad">
			...
			<tbody class="">
			<tr class="salesreturn_details"></tr>
			  <?php
			  if($update == "yes"){
			  $i = 0;
			  $sel = mysqli_query($link,"select * from salesreturndetails_tbl where salesreturn_id='$salesreturn_id'");
			  while($row_sales = mysqli_fetch_object($sel)){
			  ?>
			  <tr class="salesreturn_details" id="<?php echo $row_sales->product_id; ?>">
				<td width="30%">
					<input class="form-control" type="hidden" name="product_id[]" id="product_id<?php echo $i; ?>" value="<?php echo $row_sales->product_id; ?>">
					<input class="form-control" type="hidden" name="productcode[]" id="productcode<?php echo $i; ?>" value="<?php echo $row_sales->productcode; ?>">
					<input class="form-control" type="hidden" name="barcode[]" id="barcode<?php echo $i; ?>" value="<?php echo $row_sales->barcode; ?>">
					<textarea class="form-control" name="product_name[]" id="product_name<?php echo $i; ?>"><?php echo $row_sales->product_name; ?></textarea>
					<table>
						<tr>
						...
						<tr>
							<td><input class="form-control" type="date" name="expiry_date[]" id="expiry_date<?php echo $i; ?>" value="<?php echo $row_sales->expiry_date; ?>"></td>
							<td><input class="form-control" type="text" name="batch_no[]" id="batch_no<?php echo $i; ?>" value="<?php echo $row_sales->batch_no; ?>"></td>
						</tr>
					</table>
				</td>
				<td width="10%">
				<input class="form-control qty" type="text" name="qty[]" id="qty<?php echo $i; ?>" value="<?php echo $row_sales->qty; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="rate[]" id="rate<?php echo $i; ?>" value="<?php echo $row_sales->rate; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="unit[]" id="unit<?php echo $i; ?>" value="<?php echo $row_sales->unit; ?>">
				</td>
				<td width="15%">
				<select name="line_discount_type[]" id="line_discount_type<?php echo $i; ?>" class="form-control" onchange="update_linetotal(<?php echo $i; ?>)">
						<option value="no discount" <?php if($row_sales->line_discount_type == "no discount"){ echo "selected"; } ?>>No Discount</option>
						<option value="flatrate" <?php if($row_sales->line_discount_type == "flatrate"){ echo "selected"; } ?>>Flat Rate</option>
						<option value="percentage" <?php if($row_sales->line_discount_type == "percentage"){ echo "selected"; } ?>>Percentage</option>
				</select>
				<input class="form-control" type="text" name="line_discount[]" id="line_discount<?php echo $i; ?>" placeholder="Value" value="<?php echo $row_sales->line_discount; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				<input class="form-control linediscount" type="text" name="line_discount_total[]" id="line_discount_total<?php echo $i; ?>" placeholder="Total" value="<?php echo $row_sales->line_discount_total; ?>" readonly>
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="tax[]" id="tax<?php echo $i; ?>" value="<?php echo $row_sales->tax; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				<input class="form-control taxvalue" type="text" name="tax_total[]" id="tax_total<?php echo $i; ?>" value="<?php echo $row_sales->tax_total; ?>">
				</td>
				<td width="15%">
				<input class="form-control price" type="text" name="line_total[]" id="line_total<?php echo $i; ?>" value="<?php echo $row_sales->line_total; ?>">
				</td>
				<td>
				<a class="salesdel" href="javascript:;" title="Remove row"><i class="fa fa-close"></i></a>
				</td>
			</tr>
				<?php $i++; } }?>
			</tbody>
...
$("#customer_name option[value='<?php echo $row->customer_name; ?>']").attr("selected", "selected");
          ...
          success:function(data){
			...
			if(batch_stock == "yes"){
				...
				$("#stock-list").one("click","tr.rows", function(e){
					...
					var rate = selling_price;
					var qty = "1";
					var discount = "0.00";
					var discounttotal = "0.00";
					var rowcount = $('.salesreturn_details').length;
					//alert(rowcount);
					...
					if(found == "no"){
						if(qty < avail_stock){
						addrow(j,product_id,productcode,product_name,barcode,expiry_date,batch_no,unit,qty,rate,tax,price_changeable,stockable);
						}
						else{
				  ...
				  success:function(data){
					var responseData = jQuery.parseJSON(data);
						if(responseData!="null"){
								...
								if(count < 1){
									if(qty < avail_stock){
										addrow(rowcount,product_id,productcode,product_name,barcode,expiry_date,batch_no,unit,qty,rate,tax,price_changeable,stockable);
										var j = rowcount+1;
									}
									else
									{
...
<script type="text/javascript" src="dist/js/salesreturndetails_tbl.js"></script>
