...
if($_POST['submit'] == "Submit"){
  ...
  $purchasereturn_id = mysqli_real_escape_string($link,$_POST['purchasereturn_id']);
  $max_id = maxOfAll("id","purchasereturn_tbl");
  $max_id=$max_id+1;
  $purchasereturn_id="pur_re-".$max_id;	
  $purchasereturn_date = mysqli_real_escape_string($link,$_POST['purchasereturn_date']);
  ...
  $type = "png";
  $rand = rand();
  $file = "barcode/purchasereturn/$purchasereturn_id"."_".date("d-m-Y")."_".$rand;
  ...
  $max_id = maxOfAll("id","purchasereturndetails_tbl");
  $max_id=$max_id+1;
  $purchasereturndetails_id="pdr-".$max_id;	
  ...
  mysqli_query($link,"INSERT INTO `purchasereturndetails_tbl` (`id`, `purchasereturndetails_id`, `company_id`, `branch_id`, `purchasereturn_id`, `purchasereturn_date`, `po_no`, `po_date`, `invoice_no`, `invoice_date`, `vendor_id`, `vendor_name`, `product_id`, `productcode`, `barcode`, `product_name`, `qty`, `rate`, `unit`, `line_discount_type`, `line_discount`, `line_discount_total`, `mrp_price`, `selling_price`, `wholesale_price`, `tax`, `tax_total`, `line_total`, `batch_no`, `expiry_date`, `sort_order`, `status`, `ip_address`, `browser`, `date`, `created_by`, `approved_by`) VALUES (NULL, '$purchasereturndetails_id', '$company_id', '$branch_id', '$purchasereturn_id', '$purchasereturn_date', '$po_no', '$po_date', '$invoice_no', '$invoice_date', '$vendor_id', '$vendor_name', '$product_id', '$productcode', '$barcode', '$product_name', '$qty', '$rate', '$unit', '$line_discount_type', '$line_discount', '$line_discount_total', '$mrp_price', '$selling_price', '$wholesale_price', '$tax', '$tax_total', '$line_total', '$batch_no', '$expiry_date', '$sort_order', 'active', '$ip_address', '$browser', '$date', '$created_by', '$approved_by')");
  
  //Stock Module
  list($count) = mysqli_fetch_row(mysqli_query($link,"select count(id) as count from stock_tbl where company_id='$company_id' and branch_id='$branch_id' and product_id='$product_id' and productcode='$productcode' and product_name='$product_name' and batch_no='$batch_no' and expiry_date='$expiry_date' and vendor_id='$vendor_id' and vendor_name='$vendor_name' and mrp_price='$mrp_price'"));
  if($count > 0){
  ...
  {	
  $max_id = maxOfAll("id","stock_tbl");
  $max_id=$max_id+1;
  $stock_id="sto-".$max_id;
  mysqli_query($link,"INSERT INTO `stock_tbl` (`id`, `stock_id`, `company_id`, `branch_id`, `product_id`, `productcode`, `barcode`, `product_name`, `qty`, `batch_no`, `expiry_date`, `vendor_id`, `vendor_name`, `mrp_price`, `cost_price`, `selling_price`, `offer_price`, `wholesale_price`, `tax`, `status`, `ip_address`, `browser`, `date`, `created_by`, `approved_by`) VALUES (NULL, '$stock_id', '$company_id', '$branch_id', '$product_id', '$productcode', '$barcode', '$product_name', '$qty', '$batch_no', '$expiry_date', '$vendor_id', '$vendor_name', '$mrp_price', '$rate', '$selling_price', '$offer_price', '$wholesale_price', '$tax', '$status', '$ip_address', '$browser', '$date', '$created_by', '$approved_by')");
  }
  ...
  //Stock Module
  $sel_qty = mysqli_query($link,"select `product_id`,`qty` from purchasereturndetails_tbl where `purchasereturn_id`='$purchasereturn_id'");
  while($row_qty = mysqli_fetch_object($sel_qty)){
    $oldproduct[] = $row_qty->product_id.",".$row_qty->qty;
  }
  //End Stock Module
  
  mysqli_query($link,"delete from purchasereturndetails_tbl where `purchasereturn_id`='$purchasereturn_id'");
  $product_id1 = $_POST['product_id'];
  $i=0;
  foreach($product_id1 as $product_id2){
  $max_id = maxOfAll("id","purchasereturndetails_tbl");
  ...
  list($count) = mysqli_fetch_row(mysqli_query($link,"select count(id) as count from stock_tbl where company_id='$company_id' and branch_id='$branch_id' and product_id='$product_id' and productcode='$productcode' and product_name='$product_name' and batch_no='$batch_no' and expiry_date='$expiry_date' and vendor_id='$vendor_id' and vendor_name='$vendor_name' and mrp_price='$mrp_price'"));
  if($count > 0){
    list($stock_id,$product_id,$avail_qty) = mysqli_fetch_row(mysqli_query($link,"select `stock_id`,`product_id`,`qty` from stock_tbl where company_id='$company_id' and branch_id='$branch_id' and product_id='$product_id' and productcode='$productcode' and product_name='$product_name' and batch_no='$batch_no' and expiry_date='$expiry_date' and vendor_id='$vendor_id' and vendor_name='$vendor_name' and mrp_price='$mrp_price'"));
    foreach($oldproduct as $pro){
      //echo $pro;
      $prod = explode(",",$pro);
      $oldproduct_id = $prod[0];
      $oldqty = $prod[1];
      if($oldproduct_id == $product_id){
        if($oldqty != $qty){
              ...
              <div class="container box-bg">
                <div class="box">
                    <div class="box-header">
                      <div class="row">
                        <div class="col-lg-6"><h3 class="box-heading"> Purchase Return<small>Add / Update  Details</small></h3></h3></div>
                        <div class="col-lg-6">
                          <div class="breadcrumb">
                            <a href="index.php" class="breadcrumb_a">Home </a> 
                            <i class="fa fa-angle-double-right angle_double_right"></i>
                            <a href="purchasereturn_view.php" class="breadcrumb_a">Purchase Return </a> 
                            <i class="fa fa-angle-double-right angle_double_right" aria-hidden="true"></i>
                            <a href="#" class="breadcrumb_a">Add / Update Purchase Return </a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div class="box-body">
                      ...
                      <form method="post" class="padding-10" enctype="multipart/form-data">
                        <?php
$purchasereturn_id = mysqli_real_escape_string($link,$_GET['purchasereturn_id']);
$update = mysqli_real_escape_string($link,$_GET['update']);
if($update == "yes"){
$sel_row = mysqli_query($link,"select * from purchasereturn_tbl where `purchasereturn_id`='$purchasereturn_id'");
$row = mysqli_fetch_object($sel_row);
$upload_id = $row->purchasereturn_id;
...
<input type="hidden" class="form-control" name="id" id="id" placeholder="Enter the Id" value="<?php echo $row->id; ?>" readonly="readonly">
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        ...
        <label class="margin-left-10" for="industry_name">Purchase Return Id</label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
        <input type="text" class="form-control" name="purchasereturn_id" id="purchasereturn_id" placeholder="Enter the Purchase Return Id" value="<?php echo $upload_id; ?>" readonly="readonly">
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 no-pad">
        <label class="margin-left-10" for="industry_name">Purchase Return Date</label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
        <input type="date" class="form-control" name="purchasereturn_date" id="purchasereturn_date" placeholder="Enter the Purchase Return Date" value="<?php echo $row->purchasereturn_date; ?>" required="required">
        </div>
      </div>
    </div>
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-pad">
		<table class="table table-bordered mg-none table1">
			...
			<tbody class="">
			<tr class="purchasereturn_details"></tr>
			  <?php
			  if($update == "yes"){
			  $i = 0;
			  $sel = mysqli_query($link,"select * from purchasereturndetails_tbl where purchasereturn_id='$purchasereturn_id'");
			  while($row_purchasereturn = mysqli_fetch_object($sel)){
			  ?>
			  <tr class="purchasereturn_details" id="<?php echo $row_purchasereturn->product_id; ?>">
				<td width="25%">
					<input class="form-control" type="hidden" name="product_id[]" id="product_id<?php echo $i; ?>" value="<?php echo $row_purchasereturn->product_id; ?>">
					<input class="form-control" type="hidden" name="productcode[]" id="productcode<?php echo $i; ?>" value="<?php echo $row_purchasereturn->productcode; ?>">
					<input class="form-control" type="hidden" name="barcode[]" id="barcode<?php echo $i; ?>" value="<?php echo $row_purchasereturn->barcode; ?>">
					<textarea class="form-control" name="product_name[]" id="product_name<?php echo $i; ?>"><?php echo $row_purchasereturn->product_name; ?></textarea>
					<table>
						<tr>
						...
						<tr>
							<td><input class="form-control" type="date" name="expiry_date[]" id="expiry_date<?php echo $i; ?>" value="<?php echo $row_purchasereturn->expiry_date; ?>"></td>
							<td><input class="form-control" type="text" name="batch_no[]" id="batch_no<?php echo $i; ?>" value="<?php echo $row_purchasereturn->batch_no; ?>"></td>
						</tr>
					</table>
				</td>
				<td width="5%">
				<input class="form-control qty" type="text" name="qty[]" id="qty<?php echo $i; ?>" value="<?php echo $row_purchasereturn->qty; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="rate[]" id="rate<?php echo $i; ?>" value="<?php echo $row_purchasereturn->rate; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="unit[]" id="unit<?php echo $i; ?>" value="<?php echo $row_purchasereturn->unit; ?>">
				</td>
				<td width="15%">
				<input class="form-control" type="text" name="mrp_price[]" id="mrp_price<?php echo $i; ?>" placeholder="MRP Price" value="<?php echo $row_purchasereturn->mrp_price; ?>">
				<input class="form-control" type="text" name="selling_price[]" id="selling_price<?php echo $i; ?>" placeholder="Selling Price" value="<?php echo $row_purchasereturn->selling_price; ?>">
				<input class="form-control" type="text" name="wholesale_price[]" id="wholesale_price<?php echo $i; ?>" placeholder="Wholesale" value="<?php echo $row_purchasereturn->wholesale_price; ?>">
				</td>
				<td width="15%">
				<select name="line_discount_type[]" id="line_discount_type<?php echo $i; ?>" class="form-control" onchange="update_linetotal(<?php echo $i; ?>)">
						<option value="no discount" <?php if($row_purchasereturn->line_discount_type == "no discount"){ echo "selected"; } ?>>No Discount</option>
						<option value="flatrate" <?php if($row_purchasereturn->line_discount_type == "flatrate"){ echo "selected"; } ?>>Flat Rate</option>
						<option value="percentage" <?php if($row_purchasereturn->line_discount_type == "percentage"){ echo "selected"; } ?>>Percentage</option>
				</select>
				<input class="form-control" type="text" name="line_discount[]" id="line_discount<?php echo $i; ?>" placeholder="Value" value="<?php echo $row_purchasereturn->line_discount; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				<input class="form-control linediscount" type="text" name="line_discount_total[]" id="line_discount_total<?php echo $i; ?>" placeholder="Total" value="<?php echo $row_purchasereturn->line_discount_total; ?>" readonly>
				</td>
				<td width="10%">
				<input class="form-control" type="text" name="tax[]" id="tax<?php echo $i; ?>" value="<?php echo $row_purchasereturn->tax; ?>" onblur="update_linetotal(<?php echo $i; ?>)">
				<input class="form-control taxvalue" type="text" name="tax_total[]" id="tax_total<?php echo $i; ?>" value="<?php echo $row_purchasereturn->tax_total; ?>">
				</td>
				<td width="10%">
				<input class="form-control price" type="text" name="line_total[]" id="line_total<?php echo $i; ?>" value="<?php echo $row_purchasereturn->line_total; ?>">
				</td>
				<td>
				<a class="purchasereturndel" href="javascript:;" title="Remove row"><i class="fa fa-close"></i></a>
				</td>
			</tr>
				<?php $i++; } }?>
			</tbody>
...
<div id="stock" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
...
function get_product(){
	...
	$.ajax({
          ...
          success:function(data){
			...
			if(batch_stock == "yes"){
				$.get("purchase_getstock.php?product_id="+product_id, function(data) {
				...
				$("#stock-list").one("click","tr.rows", function(e){
					...
					var productcode = $(this).closest('tr').children('td.productcode').text();
					...
					var rowcount = $('.purchasereturn_details').length;
					//alert(rowcount);
					for(var i=1;i<=rowcount;i++){
						//alert(i);
						var check = $('#product_id'+i).val();
						//alert(check);
						if (typeof check !== 'undefined'){
							if(product_id == $('#product_id'+i).val() && batch_no == $('#batch_no'+i).val()){
							var found = "yes";
							var j = i;
							break;
							}
					...
					if(found == "no"){
						if(qty < avail_stock){
						addrow(j,product_id,productcode,product_name,barcode,expiry_date,batch_no,unit,qty,cost_price,mrp_price,selling_price,wholesale_price,tax,price_changeable,stockable);
						}
						else{
				  ...
				  success:function(data){
					var responseData = jQuery.parseJSON(data);
						if(responseData!="null"){
								...
								if(count < 1){
									if(qty < avail_stock){
										addrow(rowcount,product_id,productcode,product_name,barcode,expiry_date,batch_no,unit,qty,cost_price,mrp_price,selling_price,wholesale_price,tax,price_changeable,stockable);
										var j = rowcount+1;
									}
									else
									{
...
<script type="text/javascript" src="assets/js/purchasereturndetails_tbl.js"></script>
