...
$floor_desc = mysqli_real_escape_string($link,$_POST['floor_desc']);
$sort_order = mysqli_real_escape_string($link,$_POST['sort_order']);
$status = mysqli_real_escape_string($link,$_POST['status']);
if(mysqli_query($link,"INSERT INTO `floor_tbl` (`id`, `company_id`, `branch_id`, `floor_id`, `floor_name`, `floor_desc`, `sort_order`, `status`, `ip_address`, `browser`, `date`, `created_by`, `approved_by`) VALUES ('$id', '$company_id', '$branch_id', '$floor_id', '$floor_name', '$floor_desc', '$sort_order', '$status', '$ip_address', '$browser', '$date', '$created_by', '$approved_by')")){
...
$sort_order = mysqli_real_escape_string($link,$_POST['sort_order']);
$status = mysqli_real_escape_string($link,$_POST['status']);
if(mysqli_query($link,"UPDATE `floor_tbl` SET `id` = '$id', `company_id` = '$company_id', `branch_id` = '$branch_id', `floor_id` = '$floor_id', `floor_name` = '$floor_name', `floor_desc` = '$floor_desc', `sort_order` = '$sort_order', `status` = '$status', `ip_address` = '$ip_address', `browser` = '$browser', `date` = '$date', `created_by` = '$created_by', `approved_by` = '$approved_by' WHERE `floor_id`='$floor_id'")){$msg[] = "Successfully Saved!";
  }
  else
  {
    $err[] = "Error in saving data!";
              ...
              <div class="container box-bg">
                <div class="box">
                    <div class="box-header">
                      <div class="row">
                        <div class="col-lg-6"><h3 class="box-heading"> Floor <small>Add / Update  Details</small></h3></h3></div>
                        <div class="col-lg-6">
                          <div class="breadcrumb">
                            <a href="index.php" class="breadcrumb_a">Home</a> 
                            <i class="fa fa-angle-double-right angle_double_right"></i>
                            <a href="floor_view.php" class="breadcrumb_a">Floor </a> 
                            <i class="fa fa-angle-double-right angle_double_right" aria-hidden="true"></i>
                            <a href="#" class="breadcrumb_a">Add / Update Floor </a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div class="box-body">
                      ...
                      <form method="post" class="padding-10" enctype="multipart/form-data">
                        <?php
$floor_id = mysqli_real_escape_string($link,$_GET['floor_id']);
  $update = mysqli_real_escape_string($link,$_GET['update']);
  if($update == "yes"){
  $sel_row = mysqli_query($link,"select * from floor_tbl where `floor_id`='$floor_id'");
  $row = mysqli_fetch_object($sel_row);
  $upload_id = $floor_id;
  }
  else
  {
  $max_id = maxOfAll("id","floor_tbl");
  $max_id=$max_id+1;
  $upload_id="flo-".$max_id;
  }
  ?>
                        <div class="row mt-3">
                          <div class="col-md-12">
...
<?php
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        ...
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
          <input type="text" class="form-control" name="floor_id" id="floor_id" placeholder="Enter the Floor Id" value="<?php echo $upload_id; ?>" readonly="readonly"  min="0" max="0" >
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 no-pad">
        <label class="margin-left-10" for="industry_name">Floor Name</label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
          <input type="text" class="form-control" name="floor_name" id="floor_name" placeholder="Enter the Floor Name" value="<?php echo $row->floor_name; ?>"  min="0" max="0" required="required">
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
      <div class="form-group row">
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 no-pad">
        <label class="margin-left-10" for="industry_name">Floor Description</label>
        </div>
        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
          <textarea class="form-control" name="floor_desc" id="floor_desc" placeholder="Enter the Floor Description"   min="0" max="0" required="required">
        <?php echo $row->floor_desc; ?></textarea>
        </div>
      </div>
    </div>
